...
test('lesson recording requires a course but no longer requires an open document', () => {
  assert.match(pageSource, /if \(!activeKnowledgeCourseId\)[\s\S]*请先从课程页面进入 PDF 阅读器/)
...
test('course-only ASR persists a recording for later document alignment', () => {
  assert.match(aiSource, /if \(context\?\.courseId\.trim\(\)\)/)
  assert.match(aiSource, /context\.documentId\?\.trim\(\)/)
  assert.match(mediaSource, /if normalized_course_id:/)
